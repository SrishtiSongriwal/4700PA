...
for i = 1:nSteps
    ...
    for k = 1:Reg.n
        ...
        if bc{k}.NumS > 0
            ...
            Eincp = 0;
            for j=1:bc{k}.NumS
                Einc = Einc + bc{k}.s(j).fct(yC{k},i,t,bc{k}.s(1).paras);
                Eincp = Eincp + bc{k}.s(j).fct(yC{k},i,t-ds/c_c/2,bc{k}.s(j).paras);
                
                Hy{k}(bc{k}.s(j).xpos,:) = Hy{k}(bc{k}.s(j).xpos,:) + dt/ds/c_mu_0*Einc;
                Ez{k}(bc{k}.s(j).xpos-1,:) = Ez{k}(bc{k}.s(j).xpos-1,:) - dt/ds/c_eps_0/c_eta_0*Eincp;
            end 
        end
